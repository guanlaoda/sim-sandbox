...
框架支持在对练过程中通过 **LLM 能力** 自动注入**真实感文本噪声**，模拟不同用户通过语音/键盘输入时的各种失真。每个画像自动生成一套噪声配置，与年龄、技术素养、沟通风格联动。

> **设计理念**: 噪声全部由大模型生成，而非硬编码规则。LLM 能理解语境，生成更自然、更多样化的噪声变体（如同音字替换、方言混用、网络梗运用等），避免了正则/字典方案的覆盖局限性。
...
| 策略 | 噪声应用方式 |
|------|-------------|
| **RuleBasedStrategy** | NoiseProfile → LLM 风格化引擎后处理（规则生成干净文本 → LLM 注入噪声） |
| **LLMAssistedStrategy** | NoiseProfile → system prompt 指令（LLM 直接生成带噪声的文本，无需后处理） |

两种策略共享 `build_noise_style_instructions()` 生成的噪声描述指令，确保噪声风格一致。
